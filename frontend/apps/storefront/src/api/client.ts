...
function flush(err: unknown, token: string|null=null) { queue.forEach(p => err ? p.reject(err) : p.resolve(token!)); queue=[] }
...
api.interceptors.response.use(r => r, async err => {
  ...
  try {
    const {data} = await axios.post('/api/v1/auth/refresh',{refresh_token:localStorage.getItem('refresh_token')})
    localStorage.setItem('token',data.access_token); localStorage.setItem('refresh_token',data.refresh_token)
    flush(null,data.access_token); orig.headers.Authorization=`Bearer ${data.access_token}`; return api(orig)
  } catch(e) { flush(e); localStorage.clear(); window.location.href='/login' }
